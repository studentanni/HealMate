    ...
    try {
      if (state === "Admin") {
        ...
        if (data.success) {
          ...
          console.log("Admin token:", data.token); // Log token for debugging

          if (data.api_key) {
            localStorage.setItem("api_key", data.api_key); // ✅ Store API key
            console.log("Admin API key:", data.api_key);
          }
        } else {
        ...
        if (data.success) {
          ...
          setDToken(data.token); // Update context with token
          console.log("Doctor token:", data.token); // Log token for debugging
          if (data.api_key) {
            localStorage.setItem("api_key", data.api_key); // ✅ Store API key
            console.log("Admin API key:", data.api_key);
          }
